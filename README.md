...
- **자격증명 암호화 저장 (보안 강화)** — Google API Key 및 네이버 비밀번호를 안전하게 암호화하여 저장. 최초 실행 시 1회만 자격증명을 입력해 두면, 이후 실행 시 입력 과정 없이 자동으로 계정을 복호화하여 로그인에 사용합니다. 평문 자격증명이 기존 설정에 남아있는 경우 자동으로 이를 감지하여 암호화 마이그레이션을 수행합니다.
...
```
%LOCALAPPDATA%\NaverBlogAutoPoster
...
- Windows 전용입니다 (비밀번호 마스킹에 `msvcrt` 사용 및 암호화 사용).
- 자격증명 암호화는 현재 Windows 사용자 계정과 컴퓨터 기기에 완전히 종속(Node-locked)되어 수행됩니다. 즉, 다른 PC나 다른 사용자 계정으로 복사된 파일은 복호화할 수 없어 안전합니다.
